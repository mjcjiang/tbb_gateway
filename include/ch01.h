...
class Image {
   public:
    union Pixel {
        ...
        template <typename T>
        Pixel(T b, T g, T r) {
            bgra[0] = (std::uint8_t)b, bgra[1] = (std::uint8_t)g,
            bgra[2] = (std::uint8_t)r, bgra[3] = 0;
        }
    };
    ...
    void fill(std::uint8_t r, std::uint8_t g, std::uint8_t b, int x = -1,
              int y = -1) {
    ...
    template <typename F>
    void fill(F f) {
        if (myData.empty()) reset(myWidth, myHeight);
   ...
   private:
    ...
    using BITMAPFILEHEADER = struct {
        std::uint16_t sizeRest;  // field is not from specification,
                                 // was added for alignemt. store size of rest
                                 // of the fields
        std::uint16_t type;
        std::uint32_t size;
...
static std::shared_ptr<Image> makeFractalImage(double magn = 2000000) {
    const std::string name =
        std::string("fractal_") + std::to_string((int)magn);
